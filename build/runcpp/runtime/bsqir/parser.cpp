...
namespace ᐸRuntimeᐳ
{
    ...
    std::optional<XFloat> BSQONParser::parseFloat()
    {
        if(this->lexer.current().tokentype == BSQONTokenType::LiteralFloat) {
            ...
            if(std::strcmp(outbuff, "Float::nan") == 0) {
                this->lexer.consume();
                return std::make_optional(XFloat{std::numeric_limits<double>::quiet_NaN()});
            }
            else if(std::strcmp(outbuff, "Float::pinfty") == 0) {
                this->lexer.consume();
                return std::make_optional(XFloat{std::numeric_limits<double>::infinity()});
            }
            else if(std::strcmp(outbuff, "Float::ninfty") == 0) {
                this->lexer.consume();
                return std::make_optional(XFloat{-std::numeric_limits<double>::infinity()});
            }
            else {
